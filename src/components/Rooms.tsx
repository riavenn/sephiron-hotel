...
const Rooms = () => {
  ...
  return (
    <section
      ...
      <div
        ref={ref}
        className={`container mx-auto transition-all duration-1000 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'} px-8 lg:px-16 xl:px-32`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-6 px-4 md:px-0">
          {roomsData.map((room, index) => (
            <div key={index} className="relative overflow-hidden rounded-lg shadow-md group aspect-[4/3] md:aspect-[4/5]">
              <Image
                src={room.image}
                alt={room.name}
                layout="fill"
              ...
              <div className="absolute top-2 right-2 bg-stone-800/80 text-white text-xs px-2 py-1 rounded-full">
                ${room.price}/night
              </div>
              <div className="absolute bottom-0 left-0 right-0 p-4 text-white flex flex-col justify-end h-full">
                ...
                <div className="pt-3">
                  <button className="w-full px-4 py-2 bg-stone-700 text-white text-sm font-semibold rounded-full hover:bg-stone-400 transition-colors duration-300">Book Now</button>
                </div>
              </div>
            </div>
          ))}
