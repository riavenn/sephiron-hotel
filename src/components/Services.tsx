...
const Services: FC = () => {
  ...
  return (
    <section
      ...
      <div
        ref={ref}
        className={`container mx-auto text-center transition-all duration-1000 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'} px-8 lg:px-16 xl:px-32`}>
        ...
        <div className="mt-12 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6 px-4 md:px-0">
          {servicesData.map((service, index) => (
            <div key={index} className="bg-stone-700 text-white p-4 rounded-lg shadow-md hover:bg-stone-600 transition-colors duration-300 flex flex-col items-center justify-center aspect-square">
              <div className="mx-auto">
                {React.cloneElement(service.icon, { size: iconSize })}
              </div>
              <h3 className="mt-3 text-sm font-semibold text-center">{service.name}</h3>
            </div>
          ))}
        </div>
      </div>
